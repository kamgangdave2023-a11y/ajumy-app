...
import { caisseRoulement, enregistrerMouvementCaisse } from '../lib/caisse';

// ── Modal Alert compatible web ─────────────────────────────────
const _rAlertQueue = [];
let _rAlertSetter = null;
function showAlert(titre, message, boutons) {
  const payload = { titre, message, boutons: boutons || [{ text: 'OK' }] };
  if (_rAlertSetter) { _rAlertSetter(payload); }
  else { _rAlertQueue.push(payload); }
}
function AlertModal() {
  const [data, setData] = React.useState(null);
  React.useEffect(() => {
    _rAlertSetter = setData;
    if (_rAlertQueue.length > 0) setData(_rAlertQueue.shift());
    return () => { _rAlertSetter = null; };
  }, []);
  if (!data) return null;
  const dismiss = (btn) => {
    setData(null);
    if (btn?.onPress) btn.onPress();
    if (_rAlertQueue.length > 0) setTimeout(() => setData(_rAlertQueue.shift()), 100);
  };
  return (
            ...
            {data.boutons.map((btn, i) => (
              <TouchableOpacity key={i} onPress={() => dismiss(btn)}
                style={{ paddingHorizontal:20, paddingVertical:10, borderRadius:10,
                  backgroundColor: btn.style === 'cancel' ? '#eee' : btn.color || '#1F3864' }}>
...
export default function RoulementScreen({ onBack }) {
  ...
  if (vue === 'alimenter')   return <AlimenterFondsScreen      onBack={() => { setVue('accueil'); loadStats(); }} />;
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <TouchableOpacity style={[styles.btnSecondary, { borderColor: '#1E7E34', borderWidth: 1 }]} onPress={() => setVue('alimenter')}>
          <Text style={[styles.btnSecondaryText, { color: '#1E7E34' }]}>💰 Alimenter le fonds de roulement</Text>
        </TouchableOpacity>
...
function DemanderRoulementScreen({ onBack }) {
  ...
  async function soumettre() {
    if (!selectedAdh) { showAlert('Sélectionnez un adhérent'); return; }
    if (nbPasChoisi <= 0) { showAlert('Choisissez au moins 1 pas'); return; }
    if (!action) { showAlert('Aucun changement', 'Modifiez le nombre de pas.'); return; }
    if (selectedAdh.liste_noire) {
      showAlert('🚫 Liste noire',
    ...
    if (nbPasChoisi > PAS_MAX) {
      showAlert('Plafond dépassé', `Maximum : ${PAS_MAX} pas.`);
      return;
    }
    ...
    if (action === 'nouvelle' || action === 'ajout') {
      ...
      if (error) { showAlert('Erreur', error.message); setSaving(false); return; }
      showAlert('✅ Demande soumise',
        action === 'nouvelle'
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        if (ln) { showAlert('🚫 Liste noire', `${item.nom} ${item.prenom} — demande impossible.`); return; }
                        selectionner(item);
                      }}>
...
function CotisationsDimancheScreen({ onBack }) {
  ...
  async function enregistrer() {
    ...
    showAlert('✅ Cotisations enregistrées',
    ...
    showAlert('💰 Remboursement total',
      ...
      [{ text: 'Annuler', style: 'cancel' },
       { text: 'Confirmer', onPress: async () => {
          ...
          await caisseRoulement.retour(montantDu, r.roulement_id, r.adherent_id);
      ...
      [{ text: 'Annuler', style: 'cancel' },
       { text: 'Sanctionner', style: 'destructive', onPress: async () => {
          ...
          await supabase.from('adherents')
            .update({ nb_dettes_en_cours: (adh?.nb_dettes_en_cours || 0) + 1 })
            .eq('adherent_id', r.adherent_id);
          showAlert('✅ Sanction enregistrée', `Dette de ${SANCTION_ABS} FCFA créée.`);
          load(); setSaving(false);
        }}
      ]);
  }
...
// ══════════════════════════════════════════════════════════════
function AlimenterFondsScreen({ onBack }) {
  const [montant, setMontant]     = useState('');
  const [description, setDesc]    = useState('Dotation bureau AJUMY');
  const [solde, setSolde]         = useState(0);
  const [saving, setSaving]       = useState(false);

  useEffect(() => {
    supabase.from('fonds_roulement').select('solde').single()
      .then(({ data }) => setSolde(data?.solde || 0));
  }, []);

  async function alimenter() {
    const m = parseInt(montant);
    if (!m || m <= 0) { showAlert('Montant invalide', 'Entrez un montant valide.'); return; }
    setSaving(true);
    const today = new Date().toISOString().split('T')[0];
    const { data: f } = await supabase.from('fonds_roulement').select('solde, fonds_id').single();
    const nouveauSolde = (f?.solde || 0) + m;
    await supabase.from('fonds_roulement').update({ solde: nouveauSolde }).eq('fonds_id', f.fonds_id);
    await supabase.from('mouvement_fonds_roulement').insert({
      type_mouvement: 'dotation_bureau',
      montant: m,
      date_mouvement: today,
      description: description || 'Dotation bureau AJUMY',
    });
    // Sortie caisse
    await enregistrerMouvementCaisse({
      type: 'sortie', source: 'sorties_roulements',
      libelle: description || 'Sorties Roulements',
      montant: m, module: 'roulement',
    });
    setSaving(false);
    showAlert('✅ Fonds alimenté', `${m.toLocaleString()} FCFA ajoutés.\nNouveau solde : ${nouveauSolde.toLocaleString()} FCFA`,
  ...
  return (
    <View style={styles.container}>
      <AlertModal />
      <Header title="💰 Alimenter le fonds" onBack={onBack} />
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <View style={[styles.statCard, { borderTopColor: '#1E7E34', marginBottom: 20 }]}>
          <Text style={[styles.statCardNum, { color: '#1E7E34' }]}>{solde.toLocaleString()} FCFA</Text>
          <Text style={styles.statCardLabel}>Solde actuel du fonds</Text>
        </View>
        <Text style={styles.inputLabel}>Montant à verser (FCFA) *</Text>
        <TextInput
          style={styles.input} value={montant} onChangeText={setMontant}
          keyboardType="numeric" placeholder="Ex: 100 000"
        />
        <Text style={styles.inputLabel}>Description</Text>
        <TextInput
          style={styles.input} value={description} onChangeText={setDesc}
          placeholder="Dotation bureau AJUMY"
        />
        <View style={[styles.calcCard, { marginTop: 16 }]}>
          <Text style={{ fontSize: 13, color: '#555', lineHeight: 20 }}>
            ℹ️ Ce montant sera déduit de la caisse et ajouté au fonds de roulement disponible pour les prêts.
          </Text>
        </View>
        <TouchableOpacity
          style={[styles.btnPrimary, { backgroundColor: '#1E7E34', opacity: saving ? 0.5 : 1, marginTop: 20 }]}
          onPress={alimenter} disabled={saving}>
          {saving
            ? <ActivityIndicator color="#fff" />
            : <Text style={styles.btnPrimaryText}>💰 Verser {montant ? parseInt(montant).toLocaleString() : '—'} FCFA au fonds</Text>}
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
